...
function normalizeDateString(str) {
  ...
  // Format Google Sheets: Date(2025,9,27)
  const m2 = /^Date\((\d{4}),\s*(\d{1,2}),\s*(\d{1,2})\)/i.exec(s);
  if (m2) {
...
function generateDuplicateKey(data) {
  ...
  const onsetNorm = normalizeDateString(data.onset || '') || (data.onset || '');
  return [nama, umur, alamat, onsetNorm].join('|');
}
...
function getKey(d) {
  // Prioritaskan UUID jika tersedia agar sinkronisasi dengan Google Sheets tidak membuat duplikat
  if (d.uuid && String(d.uuid).trim() !== '') {
    return String(d.uuid).trim();
  }
  // Fallback: gunakan kombinasi kunci duplikat berbasis identitas dan onset yang sudah dinormalisasi
  return generateDuplicateKey(d);
}
